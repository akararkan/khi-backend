...
public class OAuth2AuthenticationSuccessHandler extends SimpleUrlAuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(@NonNull HttpServletRequest request,
        ...
        if (oAuth2User == null || oAuth2User.getUser() == null) {
            throw new IOException("OAuth2 principal did not resolve to an application user");
        }
        ...
        // 3. Persist the token in the configured auth cookie
        jwtCookieService.addAuthCookie(response, token);

        // 4. Clear the OAuth2 session attributes — we don't need them anymore
        HttpSession session = request.getSession(false);
        if (session != null) {
            session.removeAttribute("SPRING_SECURITY_SAVED_REQUEST");
        }

        // 5. Redirect without exposing the token in the URL
